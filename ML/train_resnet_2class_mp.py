...
def parse_args():
    p = argparse.ArgumentParser(description="Train 2-class ResNet-style emotion model with multiprocessing")
    ...
    p.add_argument("--out", default="emotion_resnet_best.pth")
    return p.parse_args()
...
class ResidualEmotionNet(nn.Module):
    def __init__(self, input_size, num_classes=2, hidden_dim=512):
        super().__init__()
...
def main():
    ...
    print(f"  - Neutral: {class_counter[0]}")
    print(f"  - Anxious: {class_counter[1]}")
    ...
    test_loader = DataLoader(TensorDataset(torch.tensor(X_test), torch.tensor(y_test)), batch_size=args.batch_size, shuffle=False)

    model = ResidualEmotionNet(input_size=X_train.shape[1], num_classes=2).to(device)
    ...
    for epoch in range(args.epochs):
        model.train()
        run_loss, train_correct, train_total = 0.0, 0, 0
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0:
            print(
                ...
                f"VLoss: {val_loss:.4f} VAcc: {val_acc:.4f} | Best: {best_acc:.4f}"
            )
    ...
    if best_state is not None:
        report_model = ResidualEmotionNet(input_size=X_train.shape[1], num_classes=2).to(device)
